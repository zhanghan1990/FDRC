...
public:
	LLTcpClass() : TclClass("Agent/TCP/FullTcp/Sack/LLDCT") {}
	TclObject* create(int, const char*const*) {
...
static class KarunaTcpClass : public TclClass {
public:
	KarunaTcpClass() : TclClass("Agent/TCP/FullTcp/Sack/KarunaTCP") {}
	TclObject* create(int, const char*const*) {
		return (new KarunaTcpAgent());
	}
} class_karuna_full;


static class FDRCTcpClass : public TclClass {
public:
	FDRCTcpClass() : TclClass("Agent/TCP/FullTcp/Sack/FDRCTCP") {}
	TclObject* create(int, const char*const*) {
		return (new FDRCTcpAgent());
	}
} class_fdrc_full;
...
FullTcpAgent::delay_bind_init_all()
{
	...
	delay_bind_init_one("flow_size_"); // Han
...
FullTcpAgent::delay_bind_dispatch(const char *varName, const char *localName, TclObject *tracer)
{
	...
	if (delay_bind(varName, localName, "dynamic_dupack_", &dynamic_dupack_, tracer)) return TCL_OK; // Mohammad
		if (delay_bind(varName, localName, "flow_size_", &flow_size_, tracer)) return TCL_OK; // Mohammad
...
// Han: flow size should be set when init the flow in bytes
	flow_size_ = nb;
...
// 		double tleft = double(deadline)/1e6 - (now() - start_time) - byterm()*8/1e10;
		double tleft = (double)(deadline)/1e6 - (now() - start_time);
...
LLTcpAgent::slowdown(int how) {
	...
	//compute penalty 
     ...
     //printf("alpha=%lf,wc=%lf,penalty=%lf,fid=%d flowsent=%lf \n",ecnhat_alpha_,wc,penalty,fid_,flowsent);
     //printf("flowsent=%lf wc=%lf, penalty=%lf\n",flowsent,wc,penalty);
 ///    printf("penalty=%lf,wc=%lf,alpha=%lf\n",penalty,wc,ecnhat_alpha_);
	//ecnhat_alpha_ = 0.07;
	// we are in slowstart for sure if cwnd < ssthresh
	if (cwnd_ < ssthresh_) 
...
void
LLTcpAgent::delay_bind_init_all()
{
	delay_bind_init_one("LL_WCMIN_"); // Han
	delay_bind_init_one("LL_WCMAX_"); // Han
	delay_bind_init_one("LL_BMIN_"); // Han
	delay_bind_init_one("LL_BMAX_"); // Han
	SackFullTcpAgent::delay_bind_init_all();
}


int LLTcpAgent::delay_bind_dispatch(const char *varName, const char *localName, TclObject *tracer){
	if (delay_bind(varName, localName, "LL_WCMIN_", &LL_WCMIN, tracer)) return TCL_OK; // han 
	if (delay_bind(varName, localName, "LL_WCMAX_", &LL_WCMAX, tracer)) return TCL_OK; // han 
	if (delay_bind(varName, localName, "LL_BMIN_", &LL_BMIN, tracer)) return TCL_OK; // han 
	if (delay_bind(varName, localName, "LL_BMAX_", &LL_BMAX, tracer)) return TCL_OK; // han 
	return SackFullTcpAgent::delay_bind_dispatch(varName, localName, tracer);
}


void KarunaTcpAgent::recv(Packet *pkt, Handler*){
	//Shuang: cancel probe mode
	prob_mode_ = false;
	prob_count_ = 0;
...
void KarunaTcpAgent::update_advertised_window(Packet *pkt) {
	int ecnbit = hdr_flags::access(pkt)->ecnecho();
	int ackno = hdr_tcp::access(pkt)->ackno();

	double bottleneckrate = (C+0.0)*1000*1000/8.0/(maxseg_+0.0);
	// compute RTT, bottleneckrate in pkts/s, rtt in s, use estimation
	double rtt_est=int(t_rtt_)*tcp_tick_/1000+(K+0.0)/bottleneckrate;
	// compute the expected rate and aggregate rate （pkts/s)
	double remainpackets = ((double)byterm()/1.0)/((double)maxseg_);
	 
	// remaining number of packets (in number)
	double tleft = deadline/1e6 - (now() - start_time);  

	// remaining time left(in second)
	double expectedrate = 0;


	// compute the aggregate rate (pkts/s,wnd/rtt)
	double sent_time=(now() - start_time); // sent time from start (in second)
	double aggregaterate = 0;
	double ratediff=0;
	double thetarate=0;
	double fw1=0;
	
	// note, we only deal with byterm()>0 here, which means this is sender side in ns2
	if(byterm()>0){
		//printf("remainpackets=%lf\n",remainpackets);
		// deal with flows that do not have deadline
		if (tleft <=0)
			wnd_=0.01;
		else
			expectedrate = remainpackets/tleft;
		
			
		if(sent_time <=0)
		   wnd_=0.01;
		else
		   aggregaterate=windowd()/rtt_est;

		
		// for flows that do not have deadline,just use dctcp
		if(deadline==0)
		   wnd_=1000000;

		
		// compute the diff between the rates
		ratediff= expectedrate-aggregaterate;
		Z=Z+ratediff;

		//printf("fid = %d expec=%lf aggregate=%lf \n",fid_,expectedrate,aggregaterate);
		
		if(Z<0) Z=0;

		thetarate = Z*expectedrate/aggregaterate/aggregaterate;

		//printf("part1=%lf\n",thetarate*rtt_est);
		//printf("fid=%d,thetarate=%lf\n",fid_,thetarate);

		//printf("fid = %d, cwnd_ = %lf,now = %lf,remaingpackets = %lf,deadline = %lf,expected = %lf,aggragate = %lf,thetarate = %lf,bottleneck = %lf,rtt = %lf,wnd = %lf\n",
		///		fid_,cwnd_,
			///	now(),remainpackets,deadline,expectedrate,
				///aggregaterate,thetarate,bottleneckrate,rtt_est,windowd());
		//printf("fid = %d, window = %lf,remaingpackets = %lf thetarate=%lf \n",fid_,windowd(),expectedrate,thetarate);
		// compute the multiply of F*W 
		fw1 = FLast*WLast;
		//printf("fw1=%lf\n",fw1);
	}
	...
	if (!ecnhat_smooth_alpha_) 
		...
		if (ackno > ecnhat_recalc_seq) {
			...
			if(byterm()>0) {
				...
				double fw2 = FLast*WLast;
				double lamada=1.0/(bottleneckrate-(fw2-fw1)/rtt_est)/(bottleneckrate-(fw2-fw1)/rtt_est);
				double fw3=K+fw1+lamada;
				// printf("fid=%d,part1=%lf,part2=%lf,fw4=%lf,wnd_=%lf\n",
				// fid_,thetarate*rtt_est,
				// (K+fw2)*rtt_est,
				// (thetarate-fw3)*rtt_est,wnd_);
				wnd_+=(thetarate-fw3)*rtt_est;
				printf("fid=%d,diff=%lf,wnd=%lf,win=%lf\n",fid_,(thetarate-fw3)*rtt_est,wnd_,windowd());
...
void KarunaTcpAgent::delay_bind_init_all() {
	delay_bind_init_one("C_"); // Han
	delay_bind_init_one("K_"); // Han
	SackFullTcpAgent::delay_bind_init_all();
}
int KarunaTcpAgent::delay_bind_dispatch(const char *varName, const char *localName, TclObject *tracer) {
	if (delay_bind(varName, localName, "C_", &C, tracer)) return TCL_OK; // Han 
	if (delay_bind(varName, localName, "K_", &K, tracer)) return TCL_OK; // Han 
	return SackFullTcpAgent::delay_bind_dispatch(varName, localName, tracer);
}

void 
FDRCTcpAgent::delay_bind_init_all()
{
	delay_bind_init_one("DMAX_"); // Han
	delay_bind_init_one("DMIN_"); // Han
	delay_bind_init_one("Threshold_tight_"); // Han
	delay_bind_init_one("Threshold_lax_"); // Han
	delay_bind_init_one("D_MAX_"); // Han
	delay_bind_init_one("F_MAX_"); // Han
	SackFullTcpAgent::delay_bind_init_all();
}

int FDRCTcpAgent::delay_bind_dispatch(const char *varName, const char *localName, TclObject *tracer){
	if (delay_bind(varName, localName, "DMAX_", &DMAX, tracer)) return TCL_OK; // Han 
	if (delay_bind(varName, localName, "DMIN_", &DMIN, tracer)) return TCL_OK; // Han 
	if (delay_bind(varName, localName, "Threshold_tight_", &Threshold_tight, tracer)) return TCL_OK; // Han 
	if (delay_bind(varName, localName, "Threshold_lax_", &Threshold_lax, tracer)) return TCL_OK; // Han 
	if (delay_bind(varName, localName, "D_MAX_", &D_MAX, tracer)) return TCL_OK; // Han 
	if (delay_bind(varName, localName, "F_MAX_", &F_MAX, tracer)) return TCL_OK; // Han 
	return SackFullTcpAgent::delay_bind_dispatch(varName, localName, tracer);
} 
...
void FDRCTcpAgent::slowdown(int how) {
	//printf("deadline %.8lf start %.8lf deadline %d\n", now(), start_time, deadline);
	double decrease;  /* added for highspeed - sylvia */
	double win, halfwin, decreasewin;
	int slowstart = 0;
	...
	if (deadline != 0) {// flows that have deadline
		...
		if(fdt < Threshold_tight){
			...
			penalty *= (DMIN+(fdt-Threshold_tight)*(DMAX-DMIN)/(Threshold_lax-Threshold_tight));
		}
	}
...
int
FDRCTcpAgent::foutput(int seqno, int reason) {
	if (deadline != 0) {
		double tleft = deadline/1e6 - (now() - start_time);
   		if (tleft < 0 && signal_on_empty_) {
			early_terminated_ = 1;
