...
# default usage: ns simpleDummbbell.tcl [methods] [TrafficNodes] [DeadlinePercentage] [BackgroundFlows]
# ns simpleDummbbell.tcl DD-TCP-SACK 8 0.1 1
...
set DeadlinePercentage [lindex $argv 2]

# Background flows (longlive flows)
set BackgroundFlows [lindex $argv 3]
...
# Set link capacity of Karuna TCP
Agent/TCP/FullTcp/Sack/KarunaTCP set C_ 1000 ;

# Set Threshold of DAFD
...
set flowlogname $sourceAlg-[expr int($load*10)]-flow.tr
...
Agent/TCP/FullTcp/Sack/DAFDTCP set T_  60;                # in ms
...
if {[string compare $sourceAlg "Karuna-TCP-Sack"] == 0} {
    Agent/TCP set ecnhat_ true
    Agent/TCPSink set ecnhat_ true
    Agent/TCP set ecnhat_g_ $DCTCP_g_;
    set myAgent "Agent/TCP/FullTcp/Sack/KarunaTCP";
}
